...
public class EventServiceTests : BaseTestClass
{
    ...
    public async Task GetVideos_ReturnsOnlyEventVideos_ForAssignedUser()
    {
        ...
        var videos = eventService.GetVideos(testData.evt.Id, testData.user.Id).ToList();
    ...
    public async Task CreateEventAsync_SetsId_And_PersistsEvent()
    {
        ...
        var created = await eventService.CreateEventAsync(evt);
    ...
    public async Task CreateEventAsync_AddsOwnerMembershipOnce()
    {
        ...
        var ownerMemberships = SeedDbContext.AssingedToEvents.Where(a => a.EventId == created.Id && a.UserId == created.Owner).ToList();
    ...
    public async Task IsUserAssignedToEvent_ReturnsTrue_WhenAssigned()
    {
        ...
        var result = eventService.IsUserAssignedToEvent(data.evt.Id, data.user.Id);
        Assert.True(result);
    }
    ...
    public async Task IsUserAssignedToEvent_ReturnsFalse_WhenNotAssigned()
    {
        ...
        var result = eventService.IsUserAssignedToEvent(evt.Id, stranger.Id);
        Assert.False(result);
    }
    ...
    public async Task IsUserAssignedToEvent_ReturnsFalse_WhenAssignedToDifferentEvent()
    {
        ...
        var result = eventService.IsUserAssignedToEvent(evtB.Id, user.Id);
        Assert.False(result);
    }
    ...
    public async Task IsUserAssignedToEvent_ReturnsTrue_WithDuplicateMemberships()
    {
        ...
        var result = eventService.IsUserAssignedToEvent(evt.Id, user.Id);
        Assert.True(result);
    }
    ...
    public async Task GetVideos_ExcludesOtherEvents()
    {
        ...
        // Share video with B
        SeedDbContext.Add(new SharedWith { Id = Guid.NewGuid(), VideoId = videoForB.Id, UserId = user.Id, EventId = evtB.Id });
        await SeedDbContext.SaveChangesAsync(TestContext.Current.CancellationToken);

        var videos = eventService.GetVideos(evtA.Id, user.Id).ToList();
        Assert.Empty(videos);
    }
    ...
    public async Task GetVideos_ExcludesGroupAndDirectShares()
    {
        ...
        var shareGroup = new SharedWith { Id = Guid.NewGuid(), VideoId = videoToGroup.Id, UserId = user.Id, GroupId = group.Id };
        ...
        var videos = eventService.GetVideos(evt.Id, user.Id).ToList();
        Assert.Empty(videos);
    }
    ...
    public async Task GetVideos_SortsByRecordedDateTime_Descending()
    {
        ...
        var result = eventService.GetVideos(evt.Id, user.Id).ToList();
    ...
    public async Task GetVideos_IsolatedByEventId_WhenUserAssignedToMultipleEvents()
    {
        ...
        var resultA = eventService.GetVideos(evtA.Id, user.Id).ToList();
    ...
    public async Task GetVideos_OwnerSeesVideos_WhenCreatedViaService()
    {
        ...
        SeedDbContext.AddRange(video, new SharedWith { Id = Guid.NewGuid(), VideoId = video.Id, UserId = owner.Id, EventId = created.Id });
        await SeedDbContext.SaveChangesAsync(TestContext.Current.CancellationToken);

        var result = eventService.GetVideos(created.Id, owner.Id).ToList();
    ...
    public async Task GetVideos_ReturnsEmpty_WhenEventCreatedOutsideService_WithoutOwnerMembership()
    {
        ...
        var video = new VideoDataBuilder().UploadedBy(owner).Build();
        SeedDbContext.AddRange(video, new SharedWith { Id = Guid.NewGuid(), VideoId = video.Id, UserId = owner.Id, EventId = evt.Id });
        await SeedDbContext.SaveChangesAsync(TestContext.Current.CancellationToken);

        var result = eventService.GetVideos(evt.Id, owner.Id).ToList();
        Assert.Empty(result);
    }
}